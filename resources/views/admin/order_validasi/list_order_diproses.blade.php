        ...
        <div class="card p-4">
            <div class="card-body">
                ...
                <table class="table table-striped table-bordered">
                    ...
                    <tbody id="tableBody">
                        @forelse ($listOrderProses as $index => $listOrderProses)
                            <tr class="text-center">
                                <th scope="row">{{ $index + 1 }}</th>
                                <td>{{ $listOrderProses->no_invoice }}</td>
                                <td>{{ $listOrderProses->Pelanggan->user->name ?? '-' }}</td>
                                <td>{{ $listOrderProses->Service->nama_service ?? '-' }}</td>
                                <td>{{ \Carbon\Carbon::parse($listOrderProses->tanggal_order)->translatedFormat('l, d F Y') }}
                                </td>
                                <td>{{ rtrim(rtrim(number_format($listOrderProses->total_berat, 2, '.', ''), '0'), '.') }}
                                    Kg</td>
                                <td>RP. {{ $listOrderProses->total_harga }}</td>
                                <td>
                                    <!-- Button trigger modal -->
                                    <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                                        data-bs-target="#modalKonfirmasi{{ $listOrderProses->id }}">
                                        Konfirmasi
                                    </button>

                                    <!-- Modal -->
                                    <div class="modal fade" id="modalKonfirmasi{{ $listOrderProses->id }}" tabindex="-1"
                                        aria-labelledby="modalLabel{{ $listOrderProses->id }}" aria-hidden="true">
                                            ...
                                            <div class="modal-content text-center"
                                                ...
                                                <div class="modal-body">
                                                    ...
                                                    <div class="d-flex justify-content-center mt-4 gap-3">
                                                        ...
                                                        <form
                                                            action="{{ route('order.proses.konfirmasi', $listOrderProses->id) }}"
